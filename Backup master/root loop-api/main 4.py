...
from .models import (
    ...
    PublishRequest, PublishResponse,  # kept for bw-compat if you re-add publish later
...
# ====================================================================================
# BOT REPLY (bot -> human): POST /bot/reply
# ====================================================================================
    ...
    try:
        rec = supa.insert(
            "messages",
            {
                ...
                "role": "user",          # safe value for enum
                "channel": "inbox",      # safe value for enum
                "visibility": "private",
...
def debug_clear(thread_id: str, x_user_id: str = Header(..., alias="X-User-Id")):
    """
    Dev-only: delete all messages for a thread.
    Guard: only the Bot can call this (X-User-Id must equal BOT_PROFILE_ID).
    """
    if x_user_id != BOT_PROFILE_ID:
        raise HTTPException(status_code=403, detail="Bot only")
...
def debug_supa():
    ...
    try:
        test = supa.rpc("member_id_for", {"u": BOT_PROFILE_ID, "l": "e94bd651-5bac-4e39-8537-fe8c788c1475"})
